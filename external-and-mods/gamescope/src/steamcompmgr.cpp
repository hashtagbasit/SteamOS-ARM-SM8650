...

{
	...
	else
	{
		...
		if ( fit )
		{
			// Small override-redirects (dropdowns) should be included in the
			// fit box. A nested-sized override is a fullscreen wrapper — using
			// it as the source size stops 720p swapchains from scaling up.
...
paint_all( global_focus_t *pFocus, bool async )
{
		...
		if ( w )
		{
			...
			else
			{
				...
				else
				{
					...
					{
						static uint32_t s_uLastTexW = 0, s_uLastTexH = 0;
						const uint32_t uTexW = frameInfo.layers[0].tex->width();
						const uint32_t uTexH = frameInfo.layers[0].tex->height();
						if ( uTexW != s_uLastTexW || uTexH != s_uLastTexH )
						{
							s_uLastTexW = uTexW;
							s_uLastTexH = uTexH;
							xwm_log.infof( "game layer tex=%ux%u scale=%.3fx%.3f output=%ux%u nested=%dx%d scaler=%u filter=%u win=0x%lx override=%d",
								uTexW, uTexH,
								frameInfo.layers[0].scale.x, frameInfo.layers[0].scale.y,
								currentOutputWidth, currentOutputHeight,
								g_nNestedWidth, g_nNestedHeight,
								(unsigned)g_upscaleScaler, (unsigned)g_upscaleFilter,
								w ? (unsigned long)x11_win( w ) : 0ul,
								w && w->override_surface() ? 1 : 0 );
						}
					}
	...
	if ( s_NightRgbCtmBlob && g_ColorMgmt.pending.nightmode.amount < 0.01f )
	{
		for ( int i = 0; i < frameInfo.layerCount; i++ )
			maybe_apply_night_rgb_ctm( &frameInfo.layers[i] );
	}
...
void xwayland_ctx_t::DetermineAndApplyFocus( const std::vector< steamcompmgr_win_t* > &vecPossibleFocusWindows )
{
	...
	// Only Steam's UI must match the nested size. Forcing games to 1080p
	// while they still present a 720p WSI swapchain makes the X pixmap and
	// the swapchain fight for the base plane (letterbox + flicker).
...
void init_xwayland_ctx(uint32_t serverId, gamescope_xwayland_server_t *xwayland_server)
{
	...
	// Deck client probes this; DRM backend has no VR overlay forwarding.
	uint32_t unVROverlayForwardingSupported = 0;
	XChangeProperty(ctx->dpy, ctx->root, ctx->atoms.gamescopeVROverlayForwarding, XA_CARDINAL, 32, PropModeReplace,
		(unsigned char *)&unVROverlayForwardingSupported, 1 );
