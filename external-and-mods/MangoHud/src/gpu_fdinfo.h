...
private:
    ...
    std::ifstream adreno_cur_freq;
    std::ifstream adreno_load;
    void init_adreno_platform();
    int get_adreno_clock();
    int get_adreno_load();

    std::vector<std::ifstream> qualcomm_temp_streams;
    void init_qualcomm_thermal();
    int get_qualcomm_gpu_temp();
...
public:
        ...
        , drm_node(drm_node)
    {
        ...
        if (module == "i915") {
            ...
            // Render node is msm_dpu; games use drm-driver "msm" in fdinfo.
            drm_engine_type = "drm-engine-gpu";
            drm_memory_type = "drm-resident-memory";
            init_adreno_platform();
            init_kgsl();
