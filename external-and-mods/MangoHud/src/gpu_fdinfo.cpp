...
static bool is_msm_module(const std::string& module)
{
    return module == "msm" || module == "msm_drm" || module == "msm_dpu";
}

static bool drm_driver_matches(const std::string& driver, const std::string& module)
{
    if (driver == module)
        return true;

    // Qualcomm: DRM render node driver is msm_dpu, game fds report msm.
    if (is_msm_module(module) && driver == "msm")
        return true;
...
void GPU_fdinfo::find_fd()
{
    ...
    for (const auto& entry : fs::directory_iterator(path)) {
        ...
        if (
            ...
            !drm_driver_matches(driver, module) ||
            (!pci_dev.empty() && !pdev.empty() && pdev != pci_dev) ||
...
void GPU_fdinfo::init_qualcomm_thermal()
{
    qualcomm_temp_streams.clear();
    ...
    for (const auto& entry : fs::directory_iterator(base)) {
        auto hwmon_dir = entry.path();
        auto name_path = hwmon_dir / "name";
        ...
        // sm8550+ kernels expose Adreno block temps as gpuss_*_thermal.
        if (name_content.find("gpuss") == std::string::npos)
            continue;

        auto temp_path = hwmon_dir / "temp1_input";
        if (!fs::exists(temp_path))
            continue;

        qualcomm_temp_streams.emplace_back(temp_path.string());
        SPDLOG_DEBUG(
            "qualcomm: GPU temp sensor found at {} ({})",
            temp_path.string(), name_content
        );
    }
}

int GPU_fdinfo::get_qualcomm_gpu_temp()
{
    uint64_t max_temp = 0;

    for (auto& stream : qualcomm_temp_streams) {
        if (!stream.is_open())
            continue;
        ...
        max_temp = std::max(max_temp, static_cast<uint64_t>(std::stoull(temp_str)));
    }

    if (max_temp == 0)
        return 0;

    return static_cast<int>(std::lround(max_temp / 1000.f));
}
...
void GPU_fdinfo::find_hwmon_sensors()
{
    ...
    if (is_msm_module(module)) {
        init_qualcomm_thermal();
        hwmon = find_hwmon_sensor_dir("gpuss");
        if (hwmon.empty())
...
int GPU_fdinfo::get_gpu_load()
{
    if (module == "xe")
        return get_xe_load();
    else if (is_msm_module(module)) {
        int load = get_kgsl_load();
        if (load > 0)
            return load;

        load = get_adreno_load();
        if (load > 0)
            return load;
    }
...
void GPU_fdinfo::init_adreno_platform()
{
    SPDLOG_INFO("adreno: probing platform GPU sysfs for {}", module);

    try {
        // L4T / legacy tegra path (Switch root, older Jetson images)
        const std::string tegra_load = "/sys/devices/gpu.0/load";
        if (fs::exists(tegra_load)) {
            adreno_load.open(tegra_load);
            SPDLOG_DEBUG("adreno: tegra load found at {}", tegra_load);
        }

        const std::string tegra_devfreq = "/sys/devices/gpu.0/devfreq";
        if (fs::exists(tegra_devfreq)) {
            for (const auto& entry : fs::directory_iterator(tegra_devfreq)) {
                auto freq_path = entry.path() / "cur_freq";
                if (fs::exists(freq_path)) {
                    adreno_cur_freq.open(freq_path);
                    SPDLOG_DEBUG("adreno: tegra clock found at {}", freq_path.string());
                    break;
                }
            }
        }

        // Upstream adreno (Odin2 sm8550, modern Qualcomm kernels)
        if (!adreno_cur_freq.is_open() && fs::exists("/sys/class/devfreq")) {
            for (const auto& entry : fs::directory_iterator("/sys/class/devfreq")) {
                ...
                adreno_cur_freq.open(freq_path);
                SPDLOG_DEBUG("adreno: clock found at {}", freq_path.string());

                auto load_path = entry.path() / "load";
                if (!adreno_load.is_open() && fs::exists(load_path))
                    adreno_load.open(load_path);

                break;
            }
        }
    } catch (fs::filesystem_error& ex) {
        SPDLOG_WARN("adreno: {}", ex.what());
    }
}

int GPU_fdinfo::get_adreno_clock()
{
    if (!adreno_cur_freq.is_open())
        return 0;

    std::string freq_str;
    adreno_cur_freq.seekg(0);
    std::getline(adreno_cur_freq, freq_str);

    if (freq_str.empty())
        return 0;

    return std::round(std::stoull(freq_str) / 1'000'000.0);
}

int GPU_fdinfo::get_adreno_load()
{
    if (!adreno_load.is_open())
        return 0;

    std::string load_str;
    adreno_load.seekg(0);
    std::getline(adreno_load, load_str);
    ...
    // Tegra reports load in permille (0-1000)
    if (load > 100)
        load = std::round(load / 10.0);
...
void GPU_fdinfo::main_thread()
{
    while (!stop_thread) {
        ...
        // Recheck fds periodically; Qualcomm games open msm DRM fds after Vulkan init.
        {
            auto t = os_time_get_nano() / 1'000'000;
            uint64_t interval = 10'000;
            if (is_msm_module(module) && fdinfo.empty())
                interval = 500;
            if (t - fdinfo_last_update_ms >= interval) {
                find_fd();
                fdinfo_last_update_ms = t;
            }
        }
        ...
        if (is_msm_module(module)) {
            int temp = get_kgsl_temp();
            if (temp <= 0)
                temp = get_qualcomm_gpu_temp();
            if (temp <= 0 && hwmon_sensors["temp"].val > 0)
                temp = static_cast<int>(hwmon_sensors["temp"].val / 1000.f);
            metrics.temp = temp;
        } else
