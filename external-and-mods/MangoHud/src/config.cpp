...
std::string get_mangohud_config_subdir() {
    // Gaming-session binaries default under ~/.config/MangoHud/steam/
    // so Plasma/Lutris can keep ~/.config/MangoHud/ untouched.
    const char *profile = getenv("MANGOHUD_PROFILE");
    if (profile && std::string(profile) == "steam")
        return "/MangoHud/steam/";

    const std::string name = get_program_name();
    if (name == "mangoapp-steam" || name == "mangohud-steam")
        return "/MangoHud/steam/";
...
static void enumerate_config_files(std::vector<std::string>& paths) {
    const std::string mangohud_dir = get_mangohud_config_subdir();
